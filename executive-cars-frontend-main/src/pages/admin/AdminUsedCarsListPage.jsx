...
import { ConfirmationDialog, Drawer } from '../../components/ui/Overlays.jsx'
  ...
  return (
    <AdminLayout title="Used Cars List">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6 items-start sm:items-center justify-between">
        <div className="flex gap-3 flex-1 flex-wrap">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} aria-label="Search used cars"
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="range" min={500000} max={100000000} step={500000}
              value={maxPrice} onChange={e => setMaxPrice(+e.target.value)} aria-label="Maximum used-car price" className="w-28 accent-blue-600" />
          ...
          <div className="overflow-x-auto responsive-record-table-wrap">
            <table className="w-full responsive-record-table">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="responsive-record-empty text-center py-16 text-gray-400">
                  ...
                  <tr key={c._id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td data-label="Year" className="px-4 py-4 text-gray-500 text-sm">{c.year}</td>
                    <td data-label="Mileage" className="px-4 py-4 text-gray-500 text-sm">{c.km?.toLocaleString()} km</td>
                    <td data-label="Price" className="px-4 py-4 text-blue-600 font-bold text-sm">PKR {formatPKR(c.price)}</td>
                    <td data-label="Added" className="px-4 py-4 text-gray-500 text-sm">{new Date(c.createdAt).toLocaleDateString()}</td>
                    <td data-label="Actions" className="px-4 py-4">
                      <div className="flex gap-2">
                        <button type="button" onClick={() => setEditTarget(c)} aria-label={`Edit ${c.make} ${c.model}`} className="w-7 h-7 rounded-lg bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 flex items-center justify-center transition-all">
                          <Pencil className="w-3.5 h-3.5" />
                        </button>
                        <button type="button" onClick={() => setDeleteTarget(c)} aria-label={`Delete ${c.make} ${c.model}`} className="w-7 h-7 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20 flex items-center justify-center transition-all">
      ...
      <ConfirmationDialog
        ...
        title="Delete used car?"
        description={deleteTarget ? `Remove ${deleteTarget.make} ${deleteTarget.model} from used cars? This cannot be undone.` : ''}
        confirmLabel="Delete"
        dangerous
      />

      <Drawer open={Boolean(editTarget)} onClose={() => setEditTarget(null)} title="Edit used car" description={editTarget ? `${editTarget.make} ${editTarget.model}` : ''}>
        {editTarget && (
            <div className="space-y-4">
              ...
              {[
                ...
                <div key={f.key}>
                  <label htmlFor={`edit-used-${f.key}`} className="block text-sm text-gray-600 font-medium mb-1.5">{f.label}</label>
                  <input id={`edit-used-${f.key}`} type="text" inputMode={f.numeric ? 'numeric' : undefined} pattern={f.numeric ? '[0-9]*' : undefined} maxLength={f.maxLength} value={editTarget[f.key] ?? ''}
