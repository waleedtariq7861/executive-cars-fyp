...
import { Search, Plus, Pencil, Trash2, Eye, Lock } from 'lucide-react'
...
import { ConfirmationDialog, Drawer, Modal } from '../../components/ui/Overlays.jsx'
...
export default function AdminAuctionListPage() {
  ...
  return (
    <AdminLayout title="Auction List">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6 items-start sm:items-center justify-between">
        <div className="flex gap-3 flex-1">
          <div className="relative flex-1 max-w-xs">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} aria-label="Search auctions"
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto responsive-record-table-wrap">
            <table className="w-full responsive-record-table">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} className="responsive-record-empty text-center py-16 text-gray-400"><Search className="w-10 h-10 mx-auto mb-3 opacity-30" /><p>No auctions found.</p></td></tr>
                ) : filtered.map(a => {
                  ...
                  return (
                    <tr key={a._id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      <td data-label="Vehicle" className="px-4 py-4">
                      ...
                      <td data-label="Owner" className="px-4 py-4">
                      ...
                      <td data-label="Year" className="px-4 py-4 text-gray-500 text-sm">{a.year}</td>
                      <td data-label="Base price" className="px-4 py-4 text-gray-700 text-sm">PKR {formatPKR(a.basePrice)}</td>
                      <td data-label="Current bid" className="px-4 py-4 text-blue-600 font-bold text-sm">PKR {formatPKR(a.currentBid)}</td>
                      <td data-label="Auction end" className="px-4 py-4">
                      ...
                      <td data-label="Actions" className="px-4 py-4">
                        <div className="flex gap-2">
                          ...
                          <button type="button" onClick={() => setDeleteTarget(a)} aria-label={`Delete ${a.make} ${a.model} auction`} className="w-7 h-7 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20 flex items-center justify-center transition-all">
      ...
      <Drawer open={Boolean(editTarget)} onClose={() => setEditTarget(null)} title="Edit auction" description={editTarget ? `${editTarget.make} ${editTarget.model}` : ''}>
        {editTarget && (
            <div className="space-y-4">
              <AdminOwnerSelect
                id="edit-auction-owner"
              ...
              {[
                ...
                <div key={f.key}>
                  <label htmlFor={`edit-auction-${f.key}`} className="block text-sm text-gray-600 font-medium mb-1.5">{f.label}</label>
                  <input id={`edit-auction-${f.key}`} type={f.type || 'text'} inputMode={f.numeric ? 'numeric' : undefined} pattern={f.numeric ? '[0-9]*' : undefined} maxLength={f.maxLength}
