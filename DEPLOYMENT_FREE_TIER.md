...
This guide uses the GitHub `main` branch, a Vercel Vite project for the frontend, and the root `render.yaml` Blueprint for one Render Free Node API. The repository does not deploy the 1 GB ML model on Render Free. The backend can send transactional email through Resend's HTTPS API after domain verification and configuration. No custom domain is needed for the frontend, but a verified sending domain is required for email.
...
Render Free blocks outbound SMTP ports 25, 465, and 587. The backend's `EMAIL_DELIVERY_MODE=resend` sends over HTTPS instead. In [Resend Domains](https://resend.com/docs/dashboard/domains/introduction), add a domain you own (a transactional subdomain such as `account.example.com` is recommended), copy the exact DNS records Resend provides into the domain's authoritative DNS provider, and wait for sending verification. Create a Resend API key with sending permission. Set `RESEND_FROM_EMAIL` to an address on that verified domain, such as `no-reply@account.example.com`. Domain verification does not create a receiving mailbox; use an address that can receive mail if you want replies. Keep the API key out of Git. Resend's Free plan currently permits 3,000 emails/month and 100/day; check its [current pricing](https://resend.com/pricing) before relying on that quota.
...
2. Render prompts for the `sync: false` values. Set `MONGO_URI`, `CLIENT_URL`, the three Cloudinary variables, `RESEND_API_KEY`, and `RESEND_FROM_EMAIL`. `CLIENT_URL` must be the exact HTTPS primary frontend origin, without a trailing slash, for example `https://www.your-domain.com` when using a custom domain or `https://your-project.vercel.app` otherwise. Correct the value after Vercel assigns the actual URL. Render generates `JWT_SECRET` itself. The Blueprint already sets `NODE_ENV=production`, `APP_MODE=production`, `PAYMENT_MODE=disabled`, `ENABLE_DEMO_SEED=false`, `REQUIRE_CLOUDINARY=true`, and `EMAIL_DELIVERY_MODE=resend`.
   For an existing Render service created manually, add `EMAIL_DELIVERY_MODE=resend`, `RESEND_API_KEY`, and `RESEND_FROM_EMAIL` in that service's Environment page, then save and redeploy. The Blueprint does not update a manually created service.
...
4. After Resend shows the sending domain as verified and Render is redeployed, verify an inspection booking OTP and a password reset message end to end using an inbox you control. Check Resend's email activity if the application reports a delivery error. The backend accepts a Resend API response as provider acceptance; actual inbox delivery still needs this live check.
...
- Render says Free web services spin down after 15 minutes idle, take time to wake, have a temporary filesystem, and cannot send SMTP on ports 25/465/587. Resend email uses HTTPS, but still depends on a verified sending domain, a valid API key, and Resend's quota. Auction closing runs in the backend process every minute and catches up when it restarts, so closures and live updates can be delayed while asleep. [Render Free limits](https://render.com/docs/free)
